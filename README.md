...
The underlying data is derived from the public **LendingClub loan dataset** (historical peer-to-peer loan data, widely mirrored on Kaggle — search "Lending Club Loan Data"). The column schema used in this project (`instalment`, `employment_length`, `mths_since_last_major_derog`, etc.) and the overall project structure follow the conventions of a well-known "Exploratory Data Analysis: Customer Loans in Finance" coursework brief. The EDA, feature engineering, model experimentation (3 models × 5 resampling techniques × 3 preprocessing pipelines), and results analysis in this repository are my own original work built on top of that data and structure.

If you're working from the raw LendingClub data instead of the renamed course schema, you'll need to rename columns to match (e.g. `loan_amnt` → `loan_amount`, `emp_length` → `employment_length`, `installment` → `instalment`) or adjust the notebooks to use the original column names.
